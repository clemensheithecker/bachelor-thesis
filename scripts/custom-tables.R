...
predictions_to_latex <- function(df, file, title, label, footnote = NULL) {
  # Capture output from xtable
  output <- capture.output(print(
    xtable(
      df,
  ...
  if (!is.null(footnote)) {
    ...
    output_new[length(output_new) + 1] <- output_new[length(output_new)]
    output_new[length(output_new) - 2] <- output_new[length(output_new) - 2]
    output_new[length(output_new) - 1] <- "\\end{ThreePartTable}"
  }
